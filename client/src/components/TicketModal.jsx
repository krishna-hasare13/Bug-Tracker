...
const TicketModal = ({ ticket, onClose, onUpdate }) => {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', priority: 'medium', status: 'todo', assignee_id: '', attachment_url: ''
  });
  ...
  useEffect(() => {
    if (ticket) {
      setFormData({
        ...
        assignee_id: ticket.assignee_id || '', attachment_url: ticket.attachment_url || ''
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isViewer) return;
    try {
      // Handle "Unassigned" (empty string) converting to null for DB
      const payload = { ...formData, assignee_id: formData.assignee_id || null };
      await axios.put(`http://localhost:5000/api/tickets/${ticket.id}`, payload);
      ...
      onUpdate({ 
        ...ticket, 
        ...formData, 
        users: assignedUser ? { full_name: assignedUser.full_name } : null 
      });
      onClose();
  ...
  // Helper for pill colors in dropdowns
  const priorityColor = (p) => {
    if (p === 'high') return 'text-rose-600 font-bold';
        ...
        <div className="bg-white px-6 py-4 border-b border-slate-100 flex justify-between items-center flex-shrink-0 sticky top-0 z-10">
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              {ticket.project_id ? 'Ticket Details' : 'Issue Tracker'}
            </span>
            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              #{ticket.id.slice(0, 4)} <span className="text-slate-300">|</span> {isViewer ? 'View Ticket' : 'Edit Ticket'}
            </h2>
          </div>
          <button 
            onClick={onClose} 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-lg font-semibold rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all disabled:bg-transparent disabled:border-none disabled:p-0" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <select 
                    ...
                    className="w-full bg-transparent text-sm font-semibold text-slate-700 outline-none cursor-pointer disabled:cursor-default"
                  >
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <select 
                    ...
                    className={`w-full bg-transparent text-sm outline-none cursor-pointer disabled:cursor-default ${priorityColor(formData.priority)}`}
                  >
              ...
              <div>
                ...
                <textarea 
                  name="description" 
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 h-32 resize-none focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all custom-scrollbar disabled:bg-transparent disabled:border-none disabled:p-0" 
                  ...
                  <div className="relative w-full h-48 bg-slate-900 rounded-xl overflow-hidden group border border-slate-200 shadow-sm">
                    ...
                    {!isViewer && (
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button type="button" onClick={() => setFormData({...formData, attachment_url: ''})} className="bg-rose-500 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg hover:bg-rose-600 transition-colors">
                          Remove Image
                        </button>
                      </div>
                    )}
                  ...
                  !isViewer && (
                    <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer bg-slate-50 hover:bg-indigo-50 hover:border-indigo-300 transition-all group">
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <p className="text-sm text-slate-500 group-hover:text-indigo-600 font-medium">{uploading ? 'Uploading...' : 'Click to upload screenshot'}</p>
                      </div>
              ...
              {!isViewer && (
                <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                  ...
                  <button type="submit" className="px-6 py-2.5 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-colors">
                    Save Changes
                  </button>
                </div>
              )}
